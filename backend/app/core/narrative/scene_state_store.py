...
DEFAULT_SCENE_STATE_DB_PATH = os.path.join(BACKEND_DIR, "data", "scene_state.db")


class SceneStateStore:
    def __init__(self, db_path: str | None = None) -> None:
        configured_path = str(os.environ.get("DRIFT_SCENE_STATE_DB_PATH") or "").strip()
        resolved_path = db_path or configured_path or DEFAULT_SCENE_STATE_DB_PATH
    ...
    def save_state(self, player_id: str, level_id: str, state: dict) -> None:
        ...
        with self._lock:
            with self._connect() as conn:
                ...
                conn.execute(
                    """
                    INSERT INTO player_scene_state (player_id, level_id, state_json, updated_at_ms)
                    VALUES (?, ?, ?, ?)
    ...
    def load_state(self, player_id: str, level_id: str) -> Optional[dict]:
        normalized_player = str(player_id or "").strip()
        normalized_level = str(level_id or "").strip()
    ...
    def delete_state(self, player_id: str, level_id: str) -> None:
        ...
        with self._lock:
            with self._connect() as conn:
                self._ensure_schema(conn)
                conn.execute(
                    "DELETE FROM player_scene_state WHERE player_id = ? AND level_id = ?",
                    (normalized_player, normalized_level),
                )
    ...
    def delete_player_states(self, player_id: str) -> int:
        ...
        with self._lock:
            with self._connect() as conn:
                self._ensure_schema(conn)
                cursor = conn.execute(
                    "DELETE FROM player_scene_state WHERE player_id = ?",
                    (normalized_player,),
                )
...
scene_state_store = SceneStateStore()
